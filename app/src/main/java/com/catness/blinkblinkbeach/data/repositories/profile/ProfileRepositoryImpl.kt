...
class ProfileRepositoryImpl @Inject constructor(
    ...
    private val usersCollection = firestore.collection(Constants.USERS)

    override suspend fun getCurrentUser(): User {
        return try {
            val snapshot =
                usersCollection.document(firebaseAuth.uid.toString()).get().await()
    ...
    override suspend fun updateUsername(username: String): User {
        return try {
            ...
            return if (username.trim() != user.username) {
                ...
                usersCollection.document(firebaseAuth.uid.toString()).set(newUser)
                    .await()
                newUser
            } else {
                user
